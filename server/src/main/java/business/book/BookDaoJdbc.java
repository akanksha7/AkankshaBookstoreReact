...
public class BookDaoJdbc implements BookDao {
    ...
    private static final String FIND_FEATURED =
            "SELECT book_id, title, author, description, price, rating, is_public, is_featured, category_id " +
                    "FROM book " +
    ...
    public List<Book> findFeaturedBooks(int limit) {
        List<Book> books = new ArrayList<>();
        try (Connection connection = JdbcUtils.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_FEATURED)) {
            ...
            throw new BookstoreQueryDbException("Encountered a problem finding book ", e);
        }
